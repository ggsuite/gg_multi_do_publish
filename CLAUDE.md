...
Two public entry points serve `do publish`'s planning pass: **`seedMessageFor(ticketDir, defaultMergeMessage)`** (the merge-message seed: `-m` wins, else the ticket description) and **`configureRepo(repoDir, seedMessage, mergeOnly)`**, which asks the questions for a single repository and returns the answers **plus the registry baseline** its increment preview was calculated from — so the caller can predict the version this repo will publish without a second registry lookup. `configureRepo` logs no repository header; the caller owns it, because only the caller knows whether it has anything to say about that repo. `configure()` stays and is implemented through the two.
...
**The planning pass** walks the repos once, in dependency order, and for each one: decides whether it needs a release (`--publish-unchanged` forces yes; a repo carrying gg_one step progress is never skipped, because its version is already bumped and possibly uploaded; otherwise `PublishSkipCheck`), asks the version increment and merge message **only when the configuration does not already answer them** (`_configCovers` asks `PublishConfig.forRepo`, so a top-level default counts as much as a per-repo override), and predicts the version later repos resolve against: a skipped repo contributes its manifest version (exact), a publishing one `nextPatch/nextMinor/nextMajor` of the version last seen on its registry — the same arithmetic gg_one performs. One pass is enough because a repo's decision depends only on its own state and on dependencies that come *before* it. Everything undecidable (an `rc` channel, an unreadable version, an unreachable registry) predicts `null`, which makes dependents publish rather than resolve against a guess. Without a terminal, a repo that needs an answer fails with a message naming the repo, `gg do configure-publish` and `--config` — instead of hanging on a prompt nobody can answer.
